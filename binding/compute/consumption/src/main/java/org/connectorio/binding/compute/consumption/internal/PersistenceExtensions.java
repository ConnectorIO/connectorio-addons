 ...
 *
 * This file was copied from openHAB project which is licensed under Eclipse Public License (EPL-2.0)
 *
 * FIXME: Remove due to static nature and explicit copy from OH sourcetree.
 *
 ...
 * @author Łukasz Dywicki - Ported to javax.time
 */
...
public class PersistenceExtensions {
  ...
  private static String getDefaultServiceId() {
    if (registry != null) {
      ...
      if (id != null) {
        ...
        LoggerFactory.getLogger(org.eclipse.smarthome.model.persistence.extensions.PersistenceExtensions.class)
  ...
  public static DecimalType averageSince(Item item, Instant timestamp, String serviceId) {
    ...
    if (lastState != null) {
      ...
      if (thisState != null) {
        thisTimestamp = BigDecimal.valueOf((ZonedDateTime.now(timeZoneProvider.getTimeZone())).toEpochSecond());
        avgValue = (thisState.toBigDecimal().add(lastState.toBigDecimal())).divide(BigDecimal.valueOf(2),
          MathContext.DECIMAL64);
        timeSpan = thisTimestamp.subtract(lastTimestamp);
        total = total.add(avgValue.multiply(timeSpan, MathContext.DECIMAL64));
  ...
  private static Iterable<HistoricItem> getAllStatesSince(Item item, Instant timestamp, String serviceId) {
    ...
    if (service instanceof QueryablePersistenceService) {
      ...
      FilterCriteria filter = new FilterCriteria();
      filter.setBeginDate(
        ZonedDateTime.ofInstant(timestamp, timeZoneProvider.getTimeZone()));
      filter.setItemName(item.getName());
  ...
  public static Instant lastUpdate(Item item, String serviceId) {
    ...
    if (service instanceof QueryablePersistenceService) {
      ...
      Iterable<HistoricItem> result = qService.query(filter);
      if (result.iterator().hasNext()) {
        Date timestamp = result.iterator().next().getTimestamp();
