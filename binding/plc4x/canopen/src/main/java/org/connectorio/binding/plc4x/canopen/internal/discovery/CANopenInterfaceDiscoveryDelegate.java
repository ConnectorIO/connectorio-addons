...
import org.connectorio.binding.can.CANInterface;
import org.connectorio.binding.can.CANInterfaceTypes;
import org.connectorio.binding.can.discovery.CANInterfaceDiscoveryDelegate;
...
public class CANopenInterfaceDiscoveryDelegate extends AbstractDiscoveryService implements DiscoveryService,
  ...
  private static final Set<ThingTypeUID> SUPPORTED_THING_TYPES = Collections.singleton(SOCKETCAN_BRIDGE_THING_TYPE);
  ...
  public void interfaceAvailable(CANInterface iface) {
    if (CANInterfaceTypes.SOCKET_CAN.equals(iface.getType())) {
      ...
      final DiscoveryResult result = DiscoveryResultBuilder
        .create(new ThingUID(SOCKETCAN_BRIDGE_THING_TYPE, name))
