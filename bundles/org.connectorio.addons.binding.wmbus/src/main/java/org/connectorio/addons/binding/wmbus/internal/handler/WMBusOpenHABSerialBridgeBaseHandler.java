...
public class WMBusOpenHABSerialBridgeBaseHandler extends WMBusSerialBridgeBaseHandler<OpenHABSerialBridgeConfig> {

  public WMBusOpenHABSerialBridgeBaseHandler(Bridge bridge, SerialPortManager serialPortManager, DiscoveryCoordinator discoveryCoordinator) {
    ...
    scheduler.execute(new Runnable() {
      ...
      public void run() {
        ...
        if (config.dataBits != null) {
          builder.setDataBits(config.dataBits);
        }
        if (config.stopBits != null) {
          builder.setStopBits(config.stopBits);
        }
        if (config.parity != null) {
          builder.setParity(config.parity);
        }
        if (config.flowControl != null) {
          builder.setFlowControl(config.flowControl);
        }
        if (config.rts != null) {
          builder.setRts(config.rts);
        }
        try {
