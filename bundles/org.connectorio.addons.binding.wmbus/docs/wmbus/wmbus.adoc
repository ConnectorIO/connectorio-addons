...
Types are simplified at stack level to several top-level primitive kinds - Long, Double, Date, String, BCD and None.
From that point of view integrations do not need to worry about exact encoding, because due to low level type encoding limits, could change during lifetime of the meter.
...
.dsl
----
include::{examplesdir}/wmbus/test.things[]
----
...
.co7io
----
include::{examplesdir}/wmbus/things.xml[]
----