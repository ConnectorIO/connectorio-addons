 ...
 */
package org.connectorio.addons.binding.mbus.discovery;
...
/**
 * M-Bus Discovery Participant can be used to provide custom thing type mapping for specific M-Bus
 * devices.
 *
 * By default, binding will attempt to use standard encoding scheme which may not be valid for all cases.
 * Use this extension point to handle devices which rely on manufacturer specific data blocks or
 ...
 */
public interface MBusDiscoveryParticipant {

  /**
   * Attempt to identify specific device from scan of M-Bus.
   *
   ...
   */
  Optional<DiscoveryResult> discover(ThingUID bridgeUID, long timeToLive, VariableDataStructure message);
